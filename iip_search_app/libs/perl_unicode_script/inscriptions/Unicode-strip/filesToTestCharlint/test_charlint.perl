...
# CVS last revised $Date: 2003/06/25 20:52:46 $ by $Author: duerst $
...
sub CheckUTF8 {
    ...
    my ($return) = "";
    my ($st);
    print STDERR "CheckUTF8-01 has '$s'.\n";
    ...
    print STDERR "CheckUTF8-02 has '$s'.\n";
    $temp = unpack("%C*", substr($s,0,1) );
    print STDERR "That is: $temp in decimal\n";
    # forbidden bytes
    if ($s =~ /[\376\377]/) {
        &addtext (\$return, "0xFE or 0xFF byte") if !$OPTU;
    }
    print STDERR "CheckUTF8-03 has '$s'.\n";
    if ($s ne "") {
...
sub ReadCharacterDataFile {
  ...
  BASE:
    while (<BASE>) {
        ...
        if ($name =~ /^<(.*), (.*)/) {
            if (!(   /^4E00;<CJK Ideograph, First>;Lo;0;L;;;;;N;;;;;$/
                  or /^9FA5;<CJK Ideograph, Last>;Lo;0;L;;;;;N;;;;;$/
                  ...
                  or /^3400;<CJK Ideograph Extension A, First>;Lo;0;L;;;;;N;;;;;$/
...
while (<>) {
    $line++;
    print STDERR "I just read line #$line: '$_'\n";
